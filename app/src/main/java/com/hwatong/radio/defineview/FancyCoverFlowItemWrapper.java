...
public class FancyCoverFlowItemWrapper extends ViewGroup {
    ...
    protected void onDetachedFromWindow() {
    	...
    	if (wrappedViewDrawingCanvas != null) {
    		
    		wrappedViewDrawingCanvas.setBitmap(null);
    	}
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        if (changed) {
            ...
            if (this.wrappedViewBitmap == null || this.wrappedViewBitmap.getWidth() != measuredWidth 
            	...
            	this.wrappedViewBitmap = Bitmap.createBitmap(measuredWidth, measuredHeight, 
    ...
    private void remeasureChildren() {
        ...
        if (child != null) {
            ...
            float oriHeight = (originalChildHeight * (1 - this.imageReflectionRatio) - reflectionGap);
            this.originalScaledownFactor = this.isReflectionEnabled ? oriHeight / originalChildHeight : 1.0f;
    ...
    private void createReflectedImages() {
        ...
        final Bitmap invertedBitmap = Bitmap.createBitmap(this.wrappedViewBitmap, 0, invertedBitmapSourceTop, 
        		                                          width, invertedHeight, matrix, true);

        this.wrappedViewDrawingCanvas.drawBitmap(invertedBitmap, 0, scaledDownHeight + reflectionGap, null);

        final Paint paint = new Paint();
        final LinearGradient shader = new LinearGradient(0, height * imageReflectionRatio + reflectionGap, 
        		                                         0, height, 0x70ffffff, 0x00ffffff, 
